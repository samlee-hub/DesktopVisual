...
v1.0.4 adds a Visual Studio C++ complex IDE workflow contract for `SingleTestProject`. VS launch must be visible desktop icon double-click, project open must be visible VS UI, files must be added through visible IDE UI, code edits must occur through the VS editor, build/run must use VS UI or visible IDE shortcuts, and output must be verified from a visible console/output surface. Backend project/file/build/run substitution is invalid.
