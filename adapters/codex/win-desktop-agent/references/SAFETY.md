...
## v1.0.1 Safety Boundary

DesktopVisual v1.0.1 keeps the local safety policy at:
...
The public baseline ships with the same conservative default boundary: explicit target titles, process allowlists, read/write roots limited to the project and TestWindow paths, and no unrestricted absolute screen clicking. A public clone must not be treated as permission to control arbitrary windows.
...
`allow_absolute_screen_click=false` records this boundary explicitly. There is still no command that accepts unrestricted desktop coordinates.
