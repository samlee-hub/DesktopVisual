...
### Limited Recovery Boundary

1. `run-task` may use only documented bounded recovery: refocus once, reobserve and retry locate once, recheck an expectation once, or use a declared UIA alternative when OCR is unavailable.
...
6. Continue only after the user confirms the next action.
