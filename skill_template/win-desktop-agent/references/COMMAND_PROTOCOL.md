...
Visual Studio C++ complex IDE workflows are accepted only when every UI action is visible and step-checkpointed. Required evidence includes `step_by_step_visible_execution=true`, `vs_open_method=desktop_icon_double_click`, `desktop_vs_icon_found=true`, `project_opened_by_visible_ui=true`, `close_method=top_right_x_visible_click`, `backend_project_creation_used=false`, `backend_file_creation_used=false`, `backend_file_write_used=false`, `backend_build_used=false`, `backend_run_used=false`, `old_mock_vlm_used=false`, and `output_verified=true`.

`SingleTestProject` must use the Empty Project template, default project location/settings, visible IDE file-add workflow, visible editor input, visible VS build/run, and visible output verification. Backend `.sln` open, backend source/header writes, `.vcxproj` edits, `msbuild`, direct exe runs, and one-shot batch scripts cannot support PASS.

## DesktopVisual v1.0.2 Skill Contract Addendum
...
- `visible-app-launch` first reveals and observes the desktop surface.
- It searches visible desktop icons and shortcuts through UIA, OCR, visible icon evidence, `.lnk`, `.url`, and webpage shortcut evidence.
- If a matching desktop icon or shortcut exists, Runtime opens it with real mouse movement and double-click.
...
- If the first desktop locate or double-click fails, the agent needs bounded recovery and a second desktop visible attempt, or two bounded desktop visible attempts or strict surface-impossible evidence.
...
- The v1.0.3+ normal VLM path is provider-gated real VLM assist through `vlm-capability-probe`, `vlm-assist-locate`, and `vlm-candidate-validate`.
- If Runtime reports VLM unavailable or not configured, the agent must not invent VLM results.
- Self-drawn or custom UI must continue through UIA, OCR, image/template, and visible fallback paths.
- Legacy mock VLM commands are deprecated test-only fixtures and must not be used for normal Agent work.
...
## v1.0.3.1 VLM Assist Commands

`vlm-capability-probe --provider codex-cli --session-id <id> --probe-image <png> --timeout-ms 60000 --cache true` probes real provider support once per task/session and writes a file cache. Reuse the cache; do not probe every step.

`vlm-assist-locate --provider codex-cli --session-id <id> --image <png> --target <description> --target-window-title <partial> --timeout-ms 60000 --min-confidence 0.65` asks the provider for visual locate-only candidate evidence. It never clicks, types, launches apps, executes commands, or chooses backend fallback. Required evidence includes `vlm_status`, `raw_response_path`, `parsed_json_path`, `screenshot_id` or `frame_id`, `candidate_accepted`, `candidate_rejected_reason`, `runtime_validation_passed`, `candidate_is_locate_only=true`, `vlm_action_executed=false`, and `runtime_action_executed=false`.

`vlm-candidate-validate --candidate-json <path> --image <png> --target <description> --target-window-title <partial> --min-confidence 0.65` validates candidate schema, screenshot/frame binding, image bounds, confidence, target semantics, coordinate space, provider/session/raw/parsed evidence, and active-protection safety flags. A rejected candidate is not a failure to be hidden; continue Runtime-only visible fallback discipline.

VLM is assistive perception, not control. If `VLM_UNAVAILABLE`, Runtime continues Runtime-only without fake candidates. VLM does not participate in backend fallback and must not bypass active protection. Accepted candidates can enter Runtime planning only after validation; v1.0.4 complex IDE workflows must use `RealVlmRuntimeBridge` / the real VLM bridge path plus coordinate mapping, target-window lock, Runtime visible action execution, and post-action verification before any action.
