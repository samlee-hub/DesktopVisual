...
$ArtifactRoot = Join-Path $Root 'artifacts\dev5.9.0-d_case_d_explorer_locator_fix'
$CasesRoot = Join-Path $ArtifactRoot 'cases'
$PreviousArtifactRoot = Join-Path $Root 'artifacts\dev5.9.0-c_strict_case_bdc'
...
$PreviousResults = @{
    'explorer_open_local_html_via_humanmode_flow' = 'FAIL_LOCATOR: testrepo'
}
...
function Find-LockedExplorerItem($Ctx, [string]$LockedHwnd, $ContentRect, [string]$ItemName, [string[]]$Patterns, [string]$MethodName, [string]$CandidateFileName) {
    ...
    foreach ($e in $elements) {
        ...
        if ($e.Rect.right -le $e.Rect.left -or $e.Rect.bottom -le $e.Rect.top) { continue }
        if (-not (Test-RectIntersects $e.Rect $ContentRect)) { continue }
        if ($e.ControlType -notmatch 'ListItem|DataItem|TreeItem|Text') { continue }
        ...
        if ($ItemName -match '^D:' -or $name -match '\(D:\)') {
            $x = [int]($e.Rect.left + [Math]::Min(170, [Math]::Max(90, $width * 0.35)))
            $y = [int]($e.Rect.top + [Math]::Min(24, [Math]::Max(14, ($e.Rect.bottom - $e.Rect.top) * 0.25)))
        } else {
            $y = [int](($e.Rect.top + $e.Rect.bottom) / 2)
        }
...
function Complete-Case($Ctx, $Status, $Summary, $Verification) {
    if ($Status -in @('STRICT_HUMANMODE_PASS', 'HUMANMODE_FALLBACK_PASS') -and (Test-Path $Ctx.Failure)) {
    ...
    $result = [pscustomobject]@{
        case_id = $Ctx.CaseId
        previous_result_from_v5_9_0_c = $PreviousResults[$Ctx.CaseId]
        target_result = 'STRICT_HUMANMODE_PASS'
...
function Resolve-ExplorerItemCenter($Ctx, [string]$LockedHwnd, [string]$ItemName, [string[]]$Patterns, [string]$CandidateFileName) {
    ...
    if ($ocr.ok) {
        ...
        if ($hit) {
            ...
            if ((Test-ScreenPoint $x $y) -and (Test-PointInRect $x $y $contentRect)) {
                ...
                return [pscustomobject]@{ X = $x; Y = $y; Method = 'ocr locked hwnd content locator'; CoordinateSource = 'locator_derived'; Raw = $hit }
            }
    ...
    if ($ItemName -notmatch '^D:') {
        ...
        if ($selected -and @($Patterns | Where-Object { ([string]$selected.Name) -match $_ }).Count -gt 0) {
            ...
            return [pscustomobject]@{ X = $sx; Y = $sy; Method = 'incremental_search_locator selected item'; CoordinateSource = 'locator_derived'; Raw = $selected }
        }
        ...
        Add-Locator $Ctx $ItemName 'incremental_search_locator typed current-folder selection' 'ok' $focusX $focusY @{
            typed = $ItemName
            locked_explorer_hwnd = $LockedHwnd
            content_rect = $contentRect
            note = 'SelectionItemPattern did not expose the selected row; the caller will press Enter and verify the resulting Explorer location.'
        }
        return [pscustomobject]@{
            X = $focusX
            Y = $focusY
            Method = 'incremental_search_locator typed current-folder selection'
            CoordinateSource = 'locator_derived'
            Raw = @{ typed = $ItemName; focus_x = $focusX; focus_y = $focusY }
            OpenWithEnter = $true
        }
    }
    return $null
...
function Add-ExplorerPathStep($Steps, [string]$StepName, [string]$ExpectedLocation, [string]$LockedHwnd, $Locator, [string]$ActionType, [string]$VerificationMethod, $Verification, [string]$ScreenshotBefore, [string]$ScreenshotAfter, [string]$Result) {
    $state = Get-LockedExplorerState $LockedHwnd
    $Steps.Add([pscustomobject]@{
        ...
        explorer_window_rect = $state.explorer_window_rect
        content_rect = Get-ExplorerContentRect $state.explorer_window_rect
...
function Case-D {
    ...
    @(
        ...
        "- mock_html: $MockHtml",
        "- setup_only: true",
        "- note: file creation is setup evidence only and is not used as open verification."
    ...
    $openSteps = @(
        @{ Name = 'D:'; StepName = 'open_d_drive'; Expected = 'D:\'; Patterns = @('D:', 'Data \(D:\)', 'Local Disk \(D:\)', '本地磁盘 \(D:\)'); CandidateFile = 'locator_candidates_d_drive.json'; VerifyPatterns = @('Data \(D:\) - 文件资源管理器', '在 Data \(D:\) 中搜索', '刷新.*Data \(D:\)', 'D:\\'); Failure = 'FAIL_D_DRIVE_OPEN' },
        @{ Name = 'testrepo'; StepName = 'open_testrepo'; Expected = 'D:\testrepo'; Patterns = @('^testrepo$'); CandidateFile = 'locator_candidates_testrepo.json'; VerifyPatterns = @('(?m)^testrepo$', 'testrepo - 文件资源管理器', '在 testrepo 中搜索', '(?m)^testwindow$', 'D:\\testrepo'); Failure = 'FAIL_LOCATOR_TESTREPO' },
        @{ Name = 'testwindow'; StepName = 'open_testwindow'; Expected = 'D:\testrepo\testwindow'; Patterns = @('^testwindow$'); CandidateFile = 'locator_candidates_testwindow.json'; VerifyPatterns = @('(?m)^testwindow$', 'testwindow - 文件资源管理器', '在 testwindow 中搜索', '(?m)^desktopvisual_mail_mock\.html$', 'desktopvisual_mail_mock', 'D:\\testrepo\\testwindow'); Failure = 'FAIL_LOCATOR_TESTWINDOW' },
        @{ Name = 'desktopvisual_mail_mock.html'; StepName = 'open_html_file'; Expected = 'file://D:/testrepo/testwindow/desktopvisual_mail_mock.html'; Patterns = @('^desktopvisual_mail_mock\.html$', 'desktopvisual_mail_mock'); CandidateFile = 'locator_candidates_html.json'; VerifyPatterns = @('DesktopVisual Local Mail Mock', 'Chrome', 'Edge'); Failure = 'FAIL_LOCATOR_HTML' }
    )
    foreach ($step in $openSteps) {
        ...
        $actionType = 'mouse.double_click'
        if ($center.OpenWithEnter) {
            Invoke-HumanAction $ctx 'keyboard.press' "open incremental-search selected item: $($step.Name)" $center.Method 'fallback_keyboard' $null $null @('desktop-press', '--key', 'ENTER', '--permission-mode', $PermissionMode) | Out-Null
            $actionType = 'incremental_search_enter'
        } else {
        ...
        if ($step.Name -eq 'desktopvisual_mail_mock.html') {
            ...
            Add-ExplorerPathStep $steps $step.StepName $step.Expected $lockedHwnd $center $actionType 'browser_file_url_or_title' @{ pending_browser_verification = $true } $beforeShot $afterShot 'pending'
            break
        }
        ...
        Add-ExplorerPathStep $steps $step.StepName $step.Expected $lockedHwnd $center $actionType $verify.Method $verify $beforeShot $afterShot $(if ($verify.Ok) { 'ok' } else { 'failed' })
        if (-not $verify.Ok) {
            $retryState = Get-LockedExplorerState $lockedHwnd
            $retryContent = Get-ExplorerContentRect $retryState.explorer_window_rect
            $selected = Get-SelectedExplorerItemEvidence $lockedHwnd $retryContent
            $selectedName = if ($selected) { [string]$selected.Name } else { '' }
            Add-Event $ctx 'explorer_enter_open_retry_selection_probe' 'ok' @{
                step = $step.StepName
                target = $step.Name
                selected_name = $selectedName
                locked_explorer_hwnd = $lockedHwnd
                note = 'Enter retry uses the same focused Explorer content after a locator-derived double-click no-op; no address-bar path input is used.'
            }
            Invoke-HumanAction $ctx 'keyboard.press' "open focused Explorer item after double-click no-op: $($step.Name)" 'focused Explorer item Enter open' 'fallback_keyboard' $null $null @('desktop-press', '--key', 'ENTER', '--permission-mode', $PermissionMode) | Out-Null
            Start-Sleep -Seconds 2
            $verify = Verify-ExplorerLocation $ctx $lockedHwnd "$($step.StepName)_enter_retry" $step.Expected $step.VerifyPatterns
            $afterShot = Save-CaseScreenshot $ctx ("after_$($step.StepName)_enter_retry.png")
            $steps[$steps.Count - 1].action_type = 'mouse.double_click_then_enter_focused_item'
            $steps[$steps.Count - 1].verification_method = $verify.Method
            $steps[$steps.Count - 1].verification_result = $verify
            $steps[$steps.Count - 1].screenshot_after = $afterShot
            $steps[$steps.Count - 1].result = $(if ($verify.Ok) { 'ok' } else { 'failed' })
            if (-not $verify.Ok -and $step.Name -eq 'D:') {
                $retryState = Get-LockedExplorerState $lockedHwnd
                $retryContent = Get-ExplorerContentRect $retryState.explorer_window_rect
                $focusX = [int]($retryContent.left + (($retryContent.right - $retryContent.left) * 0.40))
                $focusY = [int]($retryContent.top + (($retryContent.bottom - $retryContent.top) * 0.30))
                Invoke-HumanAction $ctx 'mouse.click' 'focus This PC devices content before incremental search for D:' 'heuristic_content_rect' 'heuristic_locator_derived' $focusX $focusY @('desktop-click', '--screen-x', "$focusX", '--screen-y', "$focusY", '--move-mode', 'fast-human', '--permission-mode', $PermissionMode) | Out-Null
                Invoke-HumanAction $ctx 'keyboard.type_text' 'This PC content incremental search: d' 'Explorer content incremental search' 'keyboard_after_mouse_focus' $null $null @('desktop-type', '--text', 'd', '--type-mode', 'demo-human', '--char-delay-ms', '35', '--permission-mode', $PermissionMode) | Out-Null
                $ctx.IncrementalSearchCount++
                Start-Sleep -Milliseconds 500
                Invoke-HumanAction $ctx 'keyboard.press' 'open D: after This PC content incremental search' 'Explorer content incremental search Enter' 'fallback_keyboard' $null $null @('desktop-press', '--key', 'ENTER', '--permission-mode', $PermissionMode) | Out-Null
                Start-Sleep -Seconds 2
                $verify = Verify-ExplorerLocation $ctx $lockedHwnd "$($step.StepName)_incremental_search_retry" $step.Expected $step.VerifyPatterns
                $afterShot = Save-CaseScreenshot $ctx ("after_$($step.StepName)_incremental_search_retry.png")
                $steps[$steps.Count - 1].action_type = 'mouse.double_click_then_enter_then_incremental_search_enter'
                $steps[$steps.Count - 1].verification_method = $verify.Method
                $steps[$steps.Count - 1].verification_result = $verify
                $steps[$steps.Count - 1].screenshot_after = $afterShot
                $steps[$steps.Count - 1].result = $(if ($verify.Ok) { 'ok' } else { 'failed' })
            ...
            if (-not $verify.Ok) {
                $steps | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath (Join-Path $ctx.Dir 'explorer_path_steps.json') -Encoding UTF8
                Fail-Case $ctx $step.Failure "Explorer location verification failed after opening $($step.Name); expected $($step.Expected)."
                return $ctx
            }
        }
    }
    @(
        ...
        '- D: drive, testrepo, testwindow, and desktopvisual_mail_mock.html are opened with real mouse double-clicks or allowed current-folder incremental-search Enter against locked Explorer content locators.',
        "- steps: $($steps.Count)",
    ...
    $steps | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath (Join-Path $ctx.Dir 'explorer_path_steps.json') -Encoding UTF8
    if ($verified) {
        Complete-Case $ctx 'STRICT_HUMANMODE_PASS' 'Opened local HTML by visible Explorer UI path: This PC fixture, D:, testrepo, testwindow, file double-click.' "Observed browser/local page title: $title"
        return $ctx
    }
...
if (-not (Test-Path -LiteralPath $PreviousArtifactRoot)) {
    ...
    "Required v5.9.0-c artifacts path is missing: $PreviousArtifactRoot" | Set-Content -LiteralPath (Join-Path $ArtifactRoot 'blocked_missing_v5_9_0_c_artifacts.md') -Encoding UTF8
    Fail "Required v5.9.0-c artifacts path is missing: $PreviousArtifactRoot"
}
Ensure-Dir $ArtifactRoot
...
$reportFiles = @{
    'dev_summary.md' = 'v5.9.0-d remains v5 and only fixes Case D Explorer content locator behavior. It does not enter v6, add VLM, change the permission model, narrow public release permissions, or auto commit.'
    'test_summary.md' = 'Required commands: build, winagent version, permission reset selftest, v5.9.0-d Case D runner, JSON/JSONL parse, Markdown fence validation, encoding/mojibake scan, COMMAND_PROTOCOL consistency, and git status snapshot.'
    'known_limits.md' = 'Case D remains current-machine GUI evidence. It depends on a real interactive desktop, Explorer UIA/OCR behavior, display scaling, visible local D: drive, and browser file association. SKIP/FAIL is not PASS.'
    'strict_humanmode_case_d_report.md' = 'Strict HumanMode Case D summary is recorded in case_summary.json, runtime_boundary_report.md, and the Case D task artifacts. PASS requires visible Explorer UI traversal and no Explorer address-bar path input.'
    'case_d_locator_fix_report.md' = 'v5.9.0-d fixes Case D by locking the foreground Explorer hwnd, deriving a content rect, limiting UIA/OCR to that content region, verifying each navigation level, and allowing current-folder incremental search.'
    'explorer_path_step_report.md' = 'Explorer path step evidence is recorded in cases\\explorer_open_local_html_via_humanmode_flow\\explorer_path_steps.json.'
    'testrepo_locator_report.md' = 'testrepo locator evidence is recorded in locator_trace.jsonl and locator_candidates_testrepo.json.'
    'regression_report.md' = 'Regression scope is intentionally limited to v5.9.0-d Case D plus required v5.9 permission selftest and static artifact validation.'
}
...
$map = @{
    'case_d_locator_fix_report.md' = 'explorer_open_local_html_via_humanmode_flow'
}
foreach ($entry in $map.GetEnumerator()) {
    ...
    @(
        "# $($entry.Value)",
        "",
        "- Previous result from v5.9.0-c: $($caseResult.previous_result_from_v5_9_0_c)",
...
Write-Host "v5.9.0-d Case D Explorer locator fix runner complete."
