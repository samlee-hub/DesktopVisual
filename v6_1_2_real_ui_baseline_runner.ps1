...
function New-RawCase([string]$CaseId) {
    ...
    foreach ($file in @('raw_command_log.jsonl', 'raw_stdout.jsonl', 'preliminary_observations.jsonl', 'context_trace.jsonl')) {
    ...
    [pscustomobject]@{
        ...
        ContextTrace = Join-Path $dir 'context_trace.jsonl'
        Sequence = 0
    }
}
...
function Add-ContextTrace($Ctx, [string]$Step, [bool]$Ok, [string]$StopCode, [string]$Reason, $Details) {
    Write-JsonLine $Ctx.ContextTrace ([pscustomobject]@{
        timestamp = (Get-Date).ToString('o')
        case_id = $Ctx.CaseId
        step = $Step
        ok = $Ok
...
function Test-LocalMailMockContextGuard($Ctx, [string]$Step) {
    $active = Get-ActiveRaw $Ctx "$Step-context-active-window"
    $title = if ($active.Json.ok -and $active.Json.data.title) { [string]$active.Json.data.title } else { '' }
    $process = if ($active.Json.ok -and $active.Json.data.process_name) { [string]$active.Json.data.process_name } else { '' }
    $normalize = $null
    if (-not [string]::IsNullOrWhiteSpace($title) -and $process -match 'msedge|chrome') {
        $normalizeJson = Join-Path $Ctx.ResultJson "$Step-browser-surface-normalize.json"
        $normalize = Invoke-WinAgentRaw $Ctx "$Step-browser-surface-normalize" @(
            'browser-surface-normalize',
            '--title', $title,
            '--mode', 'conservative',
            '--guard-result-json', $normalizeJson
        ) -AllowFailure
    }
    $textResult = if (-not [string]::IsNullOrWhiteSpace($title)) {
        Invoke-WinAgentRaw $Ctx "$Step-context-read-window-text" @('read-window-text', '--title', $title) -AllowFailure
    } else { $null }
    $text = if ($textResult -and $textResult.Stdout) { [string]$textResult.Stdout } else { '' }
    $joined = @($title, $process) -join "`n"
    $foregroundOk = ($process -match 'msedge|chrome')
    $pageOk = ($title -match 'DesktopVisual Local Mail Mock|desktopvisual_mail_mock')
    if (-not $pageOk -and [string]::IsNullOrWhiteSpace($title)) {
        $pageOk = ($text -match 'DesktopVisual Local Mail Mock|desktopvisual_mail_mock')
    }
    $wrongPage = ($joined -match 'Google\s+Search|New Tab|Search results|chrome browser for testing.*Search| - Search - | - 搜索 - ')
    $normalizeStop = if ($normalize -and $normalize.Json -and $normalize.Json.data) { [string]$normalize.Json.data.stop_code } else { '' }
    $activeProtection = ($normalizeStop -eq 'STOP_ACTIVE_PROTECTION_OR_LOGIN_BLOCK')
    $automationDetected = ($normalizeStop -eq 'STOP_AUTOMATION_DETECTED')
    $surfaceBlocking = ($normalizeStop -eq 'STOP_BROWSER_SURFACE_BLOCKING' -or $normalizeStop -eq 'STOP_LOADING_OR_OVERLAY_BLOCKING')
    $stopCode = ''
    $reason = ''
    if (-not $foregroundOk) {
        $stopCode = 'STOP_FOREGROUND_CHANGED'
        $reason = 'foreground browser process did not match Chrome/Edge before local mock mail action'
    } elseif ($activeProtection) {
        $stopCode = 'STOP_ACTIVE_PROTECTION_OR_LOGIN_BLOCK'
        $reason = 'active protection/login marker detected before local mock mail action'
    } elseif ($automationDetected) {
        $stopCode = 'STOP_AUTOMATION_DETECTED'
        $reason = 'automation/bot challenge marker detected before local mock mail action'
    } elseif ($surfaceBlocking) {
        $stopCode = $normalizeStop
        $reason = 'browser surface blocker remained before local mock mail action'
    } elseif ($wrongPage -or -not $pageOk) {
        $stopCode = 'STOP_WRONG_CONTEXT'
        $reason = 'current browser page is not DesktopVisual Local Mail Mock before local mock mail action'
    }
    ...
    $details = @{
        ...
        page_marker_ok = $pageOk
        wrong_page_detected = $wrongPage
        active_protection_detected = $activeProtection
        automation_detected = $automationDetected
        browser_surface_normalize_exit = if ($normalize) { $normalize.ExitCode } else { $null }
        browser_surface_normalize_stop_code = $normalizeStop
    }
    Add-ContextTrace $Ctx $Step $ok $stopCode $reason $details
    if (-not $ok) {
        Add-Preliminary $Ctx 'wrong_context_stop' @{
            step = $Step
            stop_code = $stopCode
            reason = $reason
            foreground_title = $title
...
function Invoke-WinAgentRaw($Ctx, [string]$Step, [string[]]$CommandArgs, [switch]$AllowFailure, [int]$CommandTimeoutSeconds = 60) {
    $Ctx.Sequence++
    ...
    $timedOut = -not $process.WaitForExit([Math]::Max(1, $CommandTimeoutSeconds) * 1000)
    if ($timedOut) {
        try {
            if (-not $process.HasExited) { $process.Kill() }
            [void]$process.WaitForExit(5000)
        } catch {
            Add-Content -LiteralPath $Ctx.StderrLog -Encoding UTF8 -Value "[$((Get-Date).ToString('o'))] $Step timeout kill failed: $($_.Exception.Message)"
        }
    }
    ...
    Write-JsonLine $Ctx.CommandLog ([pscustomobject]@{
        ...
        command_timeout_triggered = [bool]$timedOut
...
function ConvertTo-StdRegexLiteralPattern([string]$Text) {
    if ([string]::IsNullOrWhiteSpace($Text)) { return '' }
    return ($Text -replace '([\\\.\^\$\|\?\*\+\(\)\[\]\{\}])', '\$1')
}

function ConvertTo-GuardTitlePattern([string]$Title) {
    if ([string]::IsNullOrWhiteSpace($Title)) { return '' }
    $thisPcCn = -join ([char[]]@(0x6b64, 0x7535, 0x8111))
    $fileExplorerCn = -join ([char[]]@(0x6587, 0x4ef6, 0x8d44, 0x6e90, 0x7ba1, 0x7406, 0x5668))
    if ($Title.Contains($fileExplorerCn) -or $Title.Contains($thisPcCn) -or $Title -match 'File Explorer|This PC|Data \(D:\)') {
        return 'File Explorer|文件资源管理器|Data|This PC|此电脑|D:'
    }
    return ConvertTo-StdRegexLiteralPattern $Title
}

function Join-GuardTitlePattern([string[]]$Patterns) {
    $items = @()
...
function Get-ObservedCandidate($Ctx, [string]$Title, [string]$Target, [string]$ControlTypePattern, [string]$Step, $ViewportRect = $null) {
    ...
    if ($foundElements.Count -lt 1) { return $null }
    $selected = @($foundElements | Sort-Object @{ Expression = { $_.rect.top } }, @{ Expression = { $_.rect.left } } | Select-Object -First 1)[0]
...
function Invoke-TargetMouseAction($Ctx, [string]$Step, [string]$Title, $Candidate, [ValidateSet('click','double-click')]$Action, [string]$OverlayName = '', [string]$ExpectedAfterTitlePattern = '') {
    ...
    $guardTitlePattern = Join-GuardTitlePattern @((ConvertTo-GuardTitlePattern $Title), $ExpectedAfterTitlePattern)
    $guardProcessPattern = if ($Candidate.process_name) { ConvertTo-StdRegexLiteralPattern ([string]$Candidate.process_name) } else { '' }
    if ($ExpectedAfterTitlePattern -match 'Chrome|Edge|DesktopVisual Local Mail Mock|desktopvisual_mail_mock') {
        $guardProcessPattern = Join-GuardTitlePattern @($guardProcessPattern, 'chrome\.exe|msedge\.exe')
    }
    $guardBase = @(
        '--expected-title-pattern', $guardTitlePattern,
        '--require-target-rect', 'true',
        '--require-target-current', 'true',
        '--require-target-unique', 'true',
        '--require-target-inside-viewport', 'true',
        '--browser-normalize-before-action', 'true',
        '--browser-normalize-mode', 'conservative'
    )
    if (-not [string]::IsNullOrWhiteSpace($guardProcessPattern)) {
        $guardBase += @('--expected-process-pattern', $guardProcessPattern)
    }
    ...
    $move = Invoke-WinAgentRaw $Ctx "$Step-move" (@('desktop-move') + $baseArgs + $guardBase + @('--guard-result-json', (Join-Path $Ctx.ResultJson "$Step-move-context-guard.json"), '--result-json', $moveResult)) -AllowFailure
    if ($move.ExitCode -ne 0 -or -not ($move.Json -and $move.Json.ok -eq $true)) {
        Add-Preliminary $Ctx 'runtime_guard_action_blocked' @{
            step = "$Step-move"
            command = 'desktop-move'
            exit_code = $move.ExitCode
            guard_title_pattern = $guardTitlePattern
            action_executed = if ($move.Json -and $move.Json.data) { $move.Json.data.action_executed } else { $false }
            stop_code = if ($move.Json -and $move.Json.error) { $move.Json.error.code } else { '' }
        }
        return $false
    }
    ...
    $click = Invoke-WinAgentRaw $Ctx "$Step-$Action" (@($cmd) + $baseArgs + $guardBase + @('--guard-result-json', (Join-Path $Ctx.ResultJson "$Step-$Action-context-guard.json"), '--result-json', $clickResult)) -AllowFailure
    if ($click.ExitCode -ne 0 -or -not ($click.Json -and $click.Json.ok -eq $true)) {
        Add-Preliminary $Ctx 'runtime_guard_action_blocked' @{
            step = "$Step-$Action"
            command = $cmd
            exit_code = $click.ExitCode
            guard_title_pattern = $guardTitlePattern
            action_executed = if ($click.Json -and $click.Json.data) { $click.Json.data.action_executed } else { $false }
            stop_code = if ($click.Json -and $click.Json.error) { $click.Json.error.code } else { '' }
        }
        return $false
    }
    return $true
...
function New-DesktopShortcut([string]$Name, [string]$TargetPath, [string]$Arguments, [string]$WorkingDirectory) {
...
function Test-BrowserTargetPageLoaded($Ctx, [string]$ExpectedPattern, [string]$Step) {
    ...
    if (-not $browser) {
        $observed = Get-BrowserWindow $Ctx 2
        Add-Preliminary $Ctx 'browser_target_page_not_found' @{
            step = $Step
            expected_pattern = $ExpectedPattern
            reason = 'expected target browser page/window missing'
            observed_title = if ($observed) { [string]$observed.title } else { '' }
        }
        Add-ContextTrace $Ctx "$Step-target-window-context" $false 'STOP_WRONG_CONTEXT' 'expected local mock mail page was not the current browser context' @{
            expected_pattern = $ExpectedPattern
            observed_title = if ($observed) { [string]$observed.title } else { '' }
            continued_action_after_wrong_context = $false
        }
    ...
    $hasLoadError = $joined -match 'ERR_[A-Z_]+|This site can''t be reached|This page isn''t working|file wasn''t available|404\s+Not\s+Found|cannot find the file|refused to connect|DNS_PROBE_FINISHED|localhost refused to connect'
...
function Open-UrlThroughAddressBar($Ctx, [string]$Url, [string]$Step, [string]$ExpectedPattern = 'DesktopVisual Local Mail Mock') {
    $resultPath = Join-Path $Ctx.ResultJson "$Step-browser-open-url-human.json"
    $tracePath = Join-Path $Ctx.Dir "$Step-browser-open-url-human-context-trace.jsonl"
    $opened = Invoke-WinAgentRaw $Ctx "$Step-browser-open-url-human" @(
        'browser-open-url-human',
        '--url', $Url,
        '--expected-marker', $ExpectedPattern,
        '--browser', 'auto',
        '--permission-mode', $PermissionMode,
        '--guard-trace-jsonl', $tracePath,
        '--result-json', $resultPath
    ) -AllowFailure -CommandTimeoutSeconds 45
    $guardOk = ($opened.Ok -and $opened.Json -and $opened.Json.ok -and $opened.Json.data.marker_ok -eq $true -and $opened.Json.data.wrong_page_detected -eq $false)
    Add-Preliminary $Ctx 'browser_open_url_human_result' @{
        step = $Step
        url = $Url
        ok = [bool]$guardOk
        exit_code = $opened.ExitCode
        result_json = $resultPath
        guard_trace_jsonl = $tracePath
        wrong_page_detected = if ($opened.Json -and $opened.Json.data) { $opened.Json.data.wrong_page_detected } else { $null }
        marker_ok = if ($opened.Json -and $opened.Json.data) { $opened.Json.data.marker_ok } else { $null }
        runner_does_not_decide_pass = $true
    }
    if ($guardOk) {
        return $true
    }
    Add-ContextTrace $Ctx "$Step-browser-open-url-human" $false 'STOP_WRONG_CONTEXT' 'Runtime browser-open-url-human did not verify expected page marker.' @{
        url = $Url
        result_json = $resultPath
        continued_action_after_wrong_context = $false
    }
    ...
    if ($Url -match '^(file|http)://') {
        foreach ($browserCommand in @('chrome.exe', 'msedge.exe')) {
            $commandText = "$browserCommand $Url"
            Add-Preliminary $Ctx 'browser_url_bootstrap_via_real_ui_run_dialog' @{
                step = $Step
                url = $Url
                command_text = $commandText
                method = 'WIN+R command typed through real HumanMode keyboard'
            }
            Invoke-WinAgentRaw $Ctx "$Step-run-dialog-url-$($browserCommand.Replace('.','-'))" @('desktop-hotkey', '--keys', 'WIN+R', '--permission-mode', $PermissionMode) -AllowFailure | Out-Null
            Start-Sleep -Milliseconds 600
            Invoke-WinAgentRaw $Ctx "$Step-run-dialog-url-type-$($browserCommand.Replace('.','-'))" @('desktop-type', '--text', $commandText, '--type-mode', 'demo-human', '--char-delay-ms', '25', '--permission-mode', $PermissionMode) -AllowFailure | Out-Null
            Invoke-WinAgentRaw $Ctx "$Step-run-dialog-url-enter-$($browserCommand.Replace('.','-'))" @('desktop-press', '--key', 'ENTER', '--permission-mode', $PermissionMode) -AllowFailure | Out-Null
            Start-Sleep -Seconds 3
            $loaded = Test-BrowserTargetPageLoaded $Ctx $ExpectedPattern "$Step-run-dialog-url-$($browserCommand.Replace('.','-'))"
            if ($loaded.ok) {
...
function Fill-And-Send-MailMock($Ctx, [string]$Title, [string]$StepPrefix) {
    if (-not (Test-LocalMailMockContextGuard $Ctx "$StepPrefix-before-form")) { return $false }
    ...
    foreach ($field in $fields) {
        if (-not (Test-LocalMailMockContextGuard $Ctx "$StepPrefix-$($field.Name)-before-locate")) { return $false }
        ...
        if (-not (Test-LocalMailMockContextGuard $Ctx "$StepPrefix-$($field.Name)-before-click")) { return $false }
        $clickedField = Invoke-TargetMouseAction $Ctx "$StepPrefix-$($field.Name)-click" $Title $candidate 'click' "$($field.Name.ToLowerInvariant())_before_click.png"
        if (-not $clickedField) { return $false }
        if (-not (Test-LocalMailMockContextGuard $Ctx "$StepPrefix-$($field.Name)-before-type")) { return $false }
        $typeGuard = Join-Path $Ctx.ResultJson "$StepPrefix-$($field.Name)-type-context-guard.json"
        $typed = Invoke-WinAgentRaw $Ctx "$StepPrefix-$($field.Name)-type" @(
            'desktop-type',
            '--text', $field.Value,
            ...
            '--expected-title-pattern', (ConvertTo-StdRegexLiteralPattern $Title),
            '--required-marker', 'DesktopVisual|Local Mail Mock|Recipient|Subject|Body',
            '--expected-focus-marker', $field.Name,
            '--browser-normalize-before-action', 'true',
            '--browser-normalize-mode', 'conservative',
            '--guard-result-json', $typeGuard
        ) -AllowFailure
        if (-not $typed.Ok) {
            Add-Preliminary $Ctx 'runtime_context_guard_stop' @{
                step = "$StepPrefix-$($field.Name)-type"
                guard_result_json = $typeGuard
                exit_code = $typed.ExitCode
                stdout = $typed.Stdout
                continued_action_after_wrong_context = $false
    ...
    if (-not (Test-LocalMailMockContextGuard $Ctx "$StepPrefix-Send-before-locate")) { return $false }
    ...
    if (-not (Test-LocalMailMockContextGuard $Ctx "$StepPrefix-Send-before-click")) { return $false }
    $clickedSend = Invoke-TargetMouseAction $Ctx "$StepPrefix-Send-click" $Title $send 'click' 'send_button_before_click.png'
    if (-not $clickedSend) { return $false }
...
function Case-A {
    $ctx = New-RawCase 'v6_1_2_explorer_real_ui_sanity'
    ...
    Add-Preliminary $ctx 'setup_this_pc_fixture' @{
        setup_only = $true
        shortcut = $lnk
        target = 'explorer.exe shell:MyComputerFolder'
        runtime_open = 'visible desktop shortcut target rect and real mouse double-click'
    }

    $shortcut = Get-SelectedCandidateFromAdaptiveLocate $ctx 'Program Manager' $shortcutName 'ListItem' 'desktop_item' 'step_this_pc' 'explorer.exe'
    if (-not $shortcut) {
        $shortcut = Get-ObservedCandidate $ctx 'Program Manager' $shortcutName 'ListItem' 'step_this_pc'
    }
    if (-not $shortcut) {
        Add-Preliminary $ctx 'case_unverified_failure' @{ reason = 'DesktopVisual This PC Test shortcut not found' }
        return
    }
    $openedThisPc = Invoke-TargetMouseAction $ctx 'step_this_pc' 'Program Manager' $shortcut 'double-click' 'explorer_this_pc_before_click.png' 'This PC|File Explorer|文件资源管理器|Data|D:'
    if (-not $openedThisPc) {
        Add-Preliminary $ctx 'case_unverified_failure' @{ reason = 'Runtime guard blocked This PC shortcut double-click'; target = $shortcutName }
        return
    }
    Start-Sleep -Seconds 2
    $explorer = Get-WindowByTitlePattern $ctx 'This PC|File Explorer|Data|D:' 12 'case-d-wait-explorer'
    if (-not $explorer) {
    ...
    $steps = @(
        @{ Key = 'd_drive'; Target = 'D:'; Overlay = 'explorer_d_drive_before_click.png'; Expected = 'Data|D:' },
        @{ Key = 'testrepo'; Target = 'testrepo'; Overlay = 'explorer_testrepo_before_click.png'; Expected = 'testrepo' },
        @{ Key = 'testwindow'; Target = 'testwindow'; Overlay = 'explorer_testwindow_before_click.png'; Expected = 'testwindow' },
        @{ Key = 'html'; Target = 'desktopvisual_mail_mock.html'; Overlay = 'explorer_html_before_double_click.png'; Expected = 'DesktopVisual Local Mail Mock|desktopvisual_mail_mock|Chrome|Edge' }
    )
    ...
    foreach ($step in $steps) {
        ...
        $clicked = Invoke-TargetMouseAction $ctx "step_$($step.Key)" $title $candidate 'double-click' $step.Overlay $step.Expected
        if (-not $clicked) {
            Add-Preliminary $ctx 'case_unverified_failure' @{ reason = 'Runtime guard blocked Explorer navigation action'; target = $step.Target; title = $title; expected = $step.Expected }
            return
        }
        Start-Sleep -Seconds 2
        $verified = Get-WindowByTitlePattern $ctx $step.Expected 8 "step_$($step.Key)-verify-window"
        if (-not $verified) {
            Add-Preliminary $ctx 'case_unverified_failure' @{ reason = 'Explorer navigation did not reach expected context'; target = $step.Target; expected = $step.Expected }
            return
        }
        $explorer = $verified
...
function Case-B {
    $ctx = New-RawCase 'v6_1_2_browser_local_mail_mock_real_ui_sanity'
    Add-Preliminary $ctx 'case_started' @{ case_id = $ctx.CaseId; output = 'raw evidence only' }
    $url = 'file:///D:/testrepo/testwindow/desktopvisual_mail_mock.html'
    $opened = Open-UrlThroughAddressBar $ctx $url 'case-b-open-file-url'
    if (-not $opened) {
        Add-Preliminary $ctx 'case_unverified_failure' @{ reason = 'could not open file URL through browser address bar' }
        return
    }
    $loaded = Test-BrowserTargetPageLoaded $ctx 'DesktopVisual Local Mail Mock' 'case-b-page-confirmed'
    if (-not $loaded.ok) {
        Add-Preliminary $ctx 'case_unverified_failure' @{ reason = 'target browser page not verified after file URL'; observed_title = $loaded.title }
        return
    }
    $browser = $loaded.browser
    Invoke-WindowRelocation $ctx $browser.title 'case-b-relocate'
    $loaded = Test-BrowserTargetPageLoaded $ctx 'DesktopVisual Local Mail Mock' 'case-b-page-confirmed-after-relocate'
    if (-not $loaded.ok) {
        Add-Preliminary $ctx 'case_unverified_failure' @{ reason = 'target browser page not verified after window relocation'; observed_title = $loaded.title }
        return
    }
    $browser = $loaded.browser
    Save-WindowScreenshot $ctx $browser.title 'case-b-before-fill' | Out-Null
    $ok = Fill-And-Send-MailMock $ctx $browser.title 'case-b'
    if (-not $ok) { return }
    Add-Preliminary $ctx 'case_b_round_unverified_complete' @{ title = $browser.title }
    Add-Preliminary $ctx 'case_unverified_complete' @{ case_id = $ctx.CaseId; rounds = 1 }
}

function Case-C {
    $ctx = New-RawCase 'v6_1_2_browser_local_mail_mock_repeat_run'
    Add-Preliminary $ctx 'case_started' @{ case_id = $ctx.CaseId; output = 'raw evidence only'; repeat_of = 'v6_1_2_browser_local_mail_mock_real_ui_sanity' }
    $url = 'file:///D:/testrepo/testwindow/desktopvisual_mail_mock.html'
    $opened = Open-UrlThroughAddressBar $ctx $url 'case-c-open-file-url'
    if (-not $opened) {
        Add-Preliminary $ctx 'case_unverified_failure' @{ reason = 'could not open file URL through browser address bar for repeat run' }
        return
    }
    $loaded = Test-BrowserTargetPageLoaded $ctx 'DesktopVisual Local Mail Mock' 'case-c-page-confirmed'
    if (-not $loaded.ok) {
        Add-Preliminary $ctx 'case_unverified_failure' @{ reason = 'target browser page not verified for repeat run'; observed_title = $loaded.title }
        return
    }
    $browser = $loaded.browser
    Save-WindowScreenshot $ctx $browser.title 'case-c-before-fill' | Out-Null
    $ok = Fill-And-Send-MailMock $ctx $browser.title 'case-c'
    if (-not $ok) { return }
    Add-Preliminary $ctx 'case_c_round_unverified_complete' @{ title = $browser.title }
    Add-Preliminary $ctx 'case_unverified_complete' @{ case_id = $ctx.CaseId; rounds = 1 }
}
...
function Case-D {
    $ctx = New-RawCase 'v6_1_2_localhost_mail_mock_real_ui_sanity'
    Add-Preliminary $ctx 'case_started' @{ case_id = $ctx.CaseId; output = 'raw evidence only' }
    $port = 18091
    ...
    try {
        ...
        $opened = Open-UrlThroughAddressBar $ctx $url 'case-d-localhost-open-url'
        if (-not $opened) {
            Add-Preliminary $ctx 'case_unverified_failure' @{ reason = 'could not open localhost URL through browser address bar'; port = $port }
            return
        }
        $loaded = Test-BrowserTargetPageLoaded $ctx 'DesktopVisual Local Mail Mock' 'case-d-localhost-page-confirmed'
        if (-not $loaded.ok) {
        ...
        $browser = $loaded.browser
        Save-WindowScreenshot $ctx $browser.title 'case-d-localhost-before-fill' | Out-Null
        $ok = Fill-And-Send-MailMock $ctx $browser.title 'case-d-localhost'
        if (-not $ok) { return }
        Add-Preliminary $ctx 'case_unverified_complete' @{ case_id = $ctx.CaseId; port = $port; bind = '127.0.0.1' }
    } finally {
        Add-Preliminary $ctx 'localhost_server_stop_requested' @{ port = $port; bind = '127.0.0.1'; job_id = $job.Id }
        Stop-Job -Job $job -ErrorAction SilentlyContinue | Out-Null
        Wait-Job -Job $job -Timeout 3 -ErrorAction SilentlyContinue | Out-Null
...
if ($Rounds -ge 2) {
    ...
    Add-Content -LiteralPath (Join-Path $RawRoot 'runner_notes.txt') -Encoding UTF8 -Value 'Case C repeat run was not executed because -Rounds was less than 2.'
}
Case-D

[pscustomobject]@{
    schema_version = 'v6.1.2.runner.raw'
    version = '6.1.2'
    generated_at = (Get-Date).ToString('o')
    artifact_root = $ArtifactRoot
    raw_root = $RawRoot
    runner_role = 'collect_raw_evidence_only'
    pass_or_fail_decided_by = 'v6_1_2_real_ui_baseline_verifier.ps1'
...
Write-Host 'v6.1.2 real UI baseline raw runner complete.'
