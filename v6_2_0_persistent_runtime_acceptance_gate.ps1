...
$VerifierScript = Join-Path $Root 'v6_2_0_persistent_runtime_verifier.ps1'
...
$GateJsonPath = Join-Path $ArtifactRoot 'v6_2_0_acceptance_gate_result.json'
$GateMdPath = Join-Path $ArtifactRoot 'v6_2_0_acceptance_gate_report.md'
...
function Regression-Command-Pass($Full, [string]$Name) {
    ...
    return ($entry -and [int]$entry.exit_code -eq 0)
}

New-Item -ItemType Directory -Force -Path $ArtifactRoot | Out-Null

& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $VerifierScript -Root $Root > (Join-Path $ArtifactRoot 'v6_2_0_gate_verifier.stdout.txt') 2> (Join-Path $ArtifactRoot 'v6_2_0_gate_verifier.stderr.txt')
...
if ($verifierExit -ne 0 -or -not $verifier -or $verifier.status -ne 'PASS') {
    Add-Finding $findings 'BLOCKED_V6_2_VERIFIER_NOT_PASS' 'v6.2.0 verifier did not PASS.' $VerifierJsonPath
}
if (-not $latency -or $latency.status -ne 'PASS') {
    Add-Finding $findings 'BLOCKED_LATENCY_GATE_NOT_MET' 'Latency report is missing or not PASS.' $LatencyPath
}
if (-not $full -or $full.status -ne 'PASS') {
    Add-Finding $findings 'BLOCKED_FULL_REGRESSION_NOT_RUN' 'Full regression result is missing or not PASS.' $FullRegressionPath
}
...
$acceptedV620State = (
    $version -eq '6.2.0' -and
    $agents -match 'current_trusted_version\s*[:=]\s*6\.2\.0' -and
    $agents -match 'last_completed_version\s*[:=]\s*6\.2\.0' -and
    $agents -match 'last_completed_status\s*[:=]\s*pass' -and
    $agents -match 'next_planned_version\s*[:=]\s*6\.3\.0'
)
$acceptedV630State = (
...
if (-not ($acceptedV620State -or $acceptedV630State -or $acceptedV640State -or $acceptedV650State -or $acceptedV660State -or $blockedV670RerunState -or $acceptedV670State)) {
    Add-Finding $findings 'BLOCKED_VERSION_STATE_INCONSISTENT' "VERSION/AGENTS state is '$version', expected accepted v6.2.0 or a later accepted version through v6.7.0, or v6.7.0-rerun blocked with v6.6.0 trusted." (Join-Path $Root 'VERSION')
}
if ($changelog -notmatch 'v6\.2\.0' -or $devStatus -notmatch 'v6\.2\.0') {
    Add-Finding $findings 'BLOCKED_DEVELOPMENT_DOCS_INCONSISTENT' 'CHANGELOG.md or DEVELOPMENT_STATUS.md does not document v6.2.0.' ''
}

if ($verifier -and (
    $verifier.session_lifecycle_pass -ne $true -or
    $verifier.one_shot_compatibility_pass -ne $true -or
    $verifier.ten_step_session_workflow_pass -ne $true -or
    $verifier.browser_form_session_workflow_pass -ne $true -or
    $verifier.scroll_and_locate_session_workflow_pass -ne $true -or
    $verifier.wrong_context_inside_session_pass -ne $true -or
    $verifier.cache_invalidation_pass -ne $true -or
    $verifier.latency_comparison_pass -ne $true)) {
    Add-Finding $findings 'BLOCKED_V6_2_REQUIRED_CASE_FAILED' 'One or more v6.2.0 verifier cases failed.' $VerifierJsonPath
}

$accepted = $findings.Count -eq 0
$result = [ordered]@{
    schema_version = 'v6.2.0.persistent_runtime.acceptance_gate'
    generated_at = (Get-Date).ToString('o')
    status = if ($accepted) { 'PASS' } else { 'BLOCKED' }
    accepted = [bool]$accepted
    conclusion = if ($accepted) { 'v6.2.0 accepted' } else { 'v6.2.0 blocked' }
    current_version_file = $version
    verifier_status = if ($verifier) { $verifier.status } else { 'MISSING' }
    latency_status = if ($latency) { $latency.status } else { 'MISSING' }
    full_regression_status = if ($full) { $full.status } else { 'MISSING' }
    v6_3_allowed = [bool]$accepted
    v6_2_1_allowed = $false
...
@(
    '# v6.2.0 Persistent Runtime Acceptance Gate Report',
    '',
    ...
    "- Latency: $($result.latency_status)",
    "- Full regression: $($result.full_regression_status)",
    "- v6.3 allowed: $($result.v6_3_allowed)",
    "- v6.2.1 allowed: $($result.v6_2_1_allowed)",
    '',
    '## Findings'
...
Write-Output 'V6_2_0_ACCEPTANCE_GATE_PASS'
