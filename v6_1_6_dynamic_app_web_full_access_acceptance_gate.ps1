...
# Historical four-case/full-access gate only; not used for current PASS.
param(
    [string]$Root = '',
...
$verifierScript = Join-Path $Root 'v6_1_6_dynamic_app_web_full_access_verifier.ps1'
$verifierStdout = Join-Path $CommandLogRoot '10_v6_1_6_verifier.stdout.log'
$verifierStderr = Join-Path $CommandLogRoot '10_v6_1_6_verifier.stderr.log'
...
$commandResults.Add([ordered]@{
    name = '10_v6_1_6_dynamic_app_web_full_access_verifier'
...
foreach ($row in @($commandResults.ToArray())) {
    if ($row.required -eq $true -and $row.status -ne 'PASS') {
        if ($row.name -eq '10_v6_1_6_dynamic_app_web_full_access_verifier') {
